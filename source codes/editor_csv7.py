...
class CSVColumnSelector:
    ...
    def create_widgets(self):
        ...
        ttk.Button(self.root, text="Save Settings", command=self.save_settings).grid(row=3, column=0, columnspan=3, padx=10, pady=10)
        ttk.Button(self.root, text="Load Settings", command=self.load_settings).grid(row=4, column=0, columnspan=3, padx=10, pady=10)
    ...
    def show_instructions(self):
        instructions = (
            ...
            "7. Use 'Save Settings' to save your current selections and file paths.\n"
            "8. Use 'Load Settings' to load your previously saved selections and file paths."
        )
    ...
    def load_settings(self):
        ...
        if settings_file:
            ...
            self.file1_path.set(settings.get("file1_path", ""))
            self.file2_path.set(settings.get("file2_path", ""))
            self.column_vars1 = {col: tk.BooleanVar(value=True) for col in settings.get("selected_columns1", [])}
            self.column_vars2 = {col: tk.BooleanVar(value=True) for col in settings.get("selected_columns2", [])}
